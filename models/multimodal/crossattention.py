...
class CrossAttention(nn.Module):
    def __init__(
        ...
        n_meta_data,
        n_feat_conv,
        meta_encoder_output_size=512,
        linear_layer_size=512,
    ):
        super(CrossAttention, self).__init__()
        self.meta_encoder = nn.Sequential(
            nn.Linear(n_meta_data, 256),
            nn.BatchNorm1d(256),
            nn.ReLU(),
            nn.Linear(256, meta_encoder_output_size),
            nn.BatchNorm1d(meta_encoder_output_size),
            nn.ReLU(),
        )
        self.meta_linear = nn.Linear(meta_encoder_output_size, linear_layer_size)

        self.meta_self_attention = nn.MultiheadAttention(
            embed_dim=meta_encoder_output_size,
            num_heads=meta_encoder_output_size,
            batch_first=True,
        )
        self.bn_meta = nn.LayerNorm(meta_encoder_output_size)
        self.meta_cross_attention = nn.MultiheadAttention(
            embed_dim=linear_layer_size, num_heads=2, batch_first=True
        )
        ...
        self.img_self_attention = nn.MultiheadAttention(
            embed_dim=n_feat_conv, num_heads=n_feat_conv, batch_first=True
        )
        self.img_cross_attention = nn.MultiheadAttention(
            embed_dim=linear_layer_size, num_heads=2, batch_first=True
        )
    ...
    def forward(self, img_features, meta_data):
        ...
        # cross-attention
        img_features_cross_attention, _ = self.img_cross_attention(
            att_img_projected, att_meta_data_projected, att_meta_data_projected
        )
        meta_data_cross_attention, _ = self.meta_cross_attention(
            att_meta_data_projected, att_img_projected, att_img_projected
        )
...
class CrossAttentionAdapter(nn.Module):
    def __init__(self, vision_backbone_ouput_size, n_metadata):
        super().__init__()
        self.feature_fusion = CrossAttention(
            n_metadata, vision_backbone_ouput_size, linear_layer_size=512
        )

    def forward(self, cnn_features, metadata):
        x = cnn_features.view(cnn_features.size(0), -1)
        return self.feature_fusion(x, metadata.float())