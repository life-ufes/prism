...
# in the checkpoints, since it can easily be recomputed on the fly"
...
def run_incremental_evaluation(
    ...
    for method_dir in method_dirs:
        ...
        if "bayesian" in fusion_method_str:
            continue
    ...
    csv_path = (
        results_dir / f"incremental_evaluation_raw_results_{backbone}_{metric_name}.csv"
    )
...
def _run_incremental_evaluation_fold_bayesian(
    ...
    for step_name in tqdm(
        ...
        is_feature_used_by_bayesian = True
        if step_name == "age":  # handle edge case
            ...
            is_feature_used_by_bayesian = False  # not all features are used by the Bayesian model, in this case we reuse the last metric value

        if is_feature_used_by_bayesian or last_bacc is None:
            ...
            metrics, _, _, _ = compute_metrics(preds_df)
            balanced_accuracy = metrics.get("balanced_accuracy", 0.0)
            if balanced_accuracy > 0:
                last_bacc = balanced_accuracy
        else:
            balanced_accuracy = last_bacc
...
def _run_incremental_evaluation_fold(
    ...
    for step_name in tqdm(
        ...
        if fusion_method is not None or old_metric_value is None:
            ...
            metrics = lightning_model.metrics[
                "_test"
            ]  # Actually validation metrics because we are passing val_dataloader to test() for convenience

            metric_value = metrics[metric_name].compute().item()
        else:
...
def plot_results(
    ...
    results_dir = results_root / timestamp
    filename = (
        f"incremental_evaluation_raw_results_{backbone}_{metric_name}.csv"
        if backbone
        else f"incremental_evaluation_raw_results_{metric_name}.csv"
    )
    ...
    # Plot with custom error bars for the no_metadata case
    markers = [
        "D",
        "o",
        "*",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--metric",
        type=str,
        choices=["bacc", "precision", "f1", "specificity", "auc"],
        default="bacc",
    ...
    run_incremental_evaluation(
        ...
        args.t,
        incremental_steps,
        metric_name=metrics_to_expected_names.get(args.metric, args.metric),
        backbone=args.backbone,
    )

    plot_results(Path("benchmarks/pad20/results"), args.t)
